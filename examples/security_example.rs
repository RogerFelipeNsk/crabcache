...
use crabcache::security::{AuthManager, IpFilter, RateLimiter, SecurityContext, SecurityManager};
use std::net::IpAddr;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Add more tokens
    auth_manager
        .add_token("user456".to_string(), "user1".to_string())
        .await;
    ...
    let security_manager =
        SecurityManager::new(Some(auth_manager), Some(rate_limiter), Some(ip_filter));
    ...
    println!(
        "Context after auth: authenticated={}, user={:?}",
