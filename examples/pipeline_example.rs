...
impl CrabCacheClient {
    ...
    async fn send_pipeline_batch(
        &mut self,
        commands: &[String],
    ) -> Result<Vec<String>, Box<dyn std::error::Error>> {
        // Send all commands at once
        let batch_data = commands
            .iter()
            .map(|cmd| format!("{}\n", cmd))
            .collect::<String>();
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..100 {
        let response = client
            .send_command(&format!("PUT single_key_{} single_value_{}", i, i))
            .await?;
        if response != "OK" {
    ...
    for batch_idx in 0..num_batches {
        ...
        for i in 0..batch_size {
            ...
            commands.push(format!(
                "PUT pipeline_key_{} pipeline_value_{}",
                key_idx, key_idx
            ));
        }
    ...
    println!(
        "   Successful operations: PUT={}, GET={}, DEL={}",
