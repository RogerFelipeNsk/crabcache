...
use crabcache::client::{NativeClient, ClientConfig};
...
async fn test_pipeline_operations(client: &NativeClient) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  ✅ Pipeline completed in {:.2}ms", elapsed.as_secs_f64() * 1000.0);
    ...
    println!("  ✅ {}/{} commands successful", success_count, responses.len());
    
    Ok(())
}

async fn run_performance_benchmark(client: &NativeClient) -> Result<(), Box<dyn std::error::Error>> {
    const OPERATIONS: usize = 1000;
    const BATCH_SIZE: usize = 100;
    
    println!("  Running {} operations in batches of {}...", OPERATIONS, BATCH_SIZE);
    ...
    if improvement > 0.0 {
        ...
        println!("    📉 {:.1}% slower than Phase 2 baseline", improvement.abs());
    }
...
fn show_metrics(client: &NativeClient) {
    ...
    println!("    Binary protocol usage: {:.1}%", metrics.binary_protocol_rate());
    ...
    println!("    Health check failures: {}", pool_metrics.health_check_failures);
}