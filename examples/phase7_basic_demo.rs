...
use crabcache::cluster::{
    ClusterNode, ConsistentHashRing, LoadBalancer, LoadBalancingStrategy, NodeCapabilities, NodeId,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!(
        "  Sharding stats: {} total nodes, balanced: {}",
    ...
    info!(
        "    Balancing decisions: {}",
        lb_metrics.balancing_decisions
    );
    info!(
        "    Distribution efficiency: {:.3}",
        lb_metrics.distribution_efficiency
    );
