...
async fn demo_protocol_negotiation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Client sends: {:02X?} (CRAB magic + version)", &protobuf_request[0..4]);
    ...
    println!("   Negotiated: {:?} v{}", negotiation_result.protocol, negotiation_result.version);
    println!("   Zero-copy enabled: {}", negotiation_result.zero_copy_enabled);
    println!("   Compression enabled: {}", negotiation_result.compression_enabled);
...
async fn demo_protobuf_parsing() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("     Zero-copy percentage: {:.1}%", metrics.zero_copy_percentage);
...
async fn demo_protobuf_serialization() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let responses = vec![
        ...
        ("PONG", Response::Pong),
        ("STATS", Response::Stats(r#"{"ops_per_sec":3020794,"memory_usage":"1.2GB"}"#.to_string())),
    ];
    ...
    println!("     Messages processed: {}", metrics.messages_processed);
    println!("     Average serialize time: {:.2}μs", metrics.avg_serialize_time_us);
...
async fn demo_performance_comparison() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("     Estimated Protobuf size: {} bytes", estimated_protobuf_size);
    println!("     Size reduction: {:.1}%", (1.0 - estimated_protobuf_size as f64 / json_size as f64) * 100.0);
