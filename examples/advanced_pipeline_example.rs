...
use bytes::Bytes;
use crabcache::protocol::{AdvancedPipelineConfig, AdvancedPipelineProcessor, Command, Response};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match processor.process_batch_advanced(&basic_batch).await {
        Ok(response_batch) => {
            ...
            println!(
                "✅ Processed {} commands in {:.2}ms",
                response_batch.responses.len(),
                processing_time.as_secs_f64() * 1000.0
            );

            let ops_per_second =
                response_batch.responses.len() as f64 / processing_time.as_secs_f64();
    ...
    for &batch_size in &batch_sizes {
        ...
        println!(
            "   Batch Size {}: {:.0} ops/sec (avg over 5 runs)",
    ...
    println!(
        ...
        final_metrics.total_commands
    );
    println!(
        "   Average Throughput: {:.0} ops/sec",
        final_metrics.current_throughput
    );
