...
use crate::protocol::protobuf::{
    ProtobufResult, ProtobufConfig, ProtobufMetrics,
    generated::{crab_cache_response, CrabCacheResponse, ResponseStatus, BatchResponse, OkResponse, ValueResponse, NullResponse, ErrorResponse, PongResponse, StatsResponse, ServerInfo, ProtocolCapabilities},
...
impl ProtobufSerializer {
    ...
    pub fn serialize_response(&mut self, response: Response, request_id: String) -> ProtobufResult<Bytes> {
        ...
        // Encode the message
        let mut buf = Vec::new();
        proto_response.encode(&mut buf)
            .map_err(|_| crate::protocol::protobuf::ProtobufError::DecodeError(
                prost::DecodeError::new("Stub encode error")
            ))?;
        ...
        self.metrics.update_message_processed(final_message.len(), 0.0, serialize_time);
        
        Ok(final_message)
    }
    
    /// Serialize multiple responses as a batch
    pub fn serialize_batch_response(&mut self, responses: Vec<Response>, request_ids: Vec<String>) -> ProtobufResult<Bytes> {
        let start_time = Instant::now();
    ...
    /// Convert internal response to protobuf response
    fn convert_response_to_proto(&self, response: Response, request_id: String) -> ProtobufResult<CrabCacheResponse> {
...
mod tests {
    ...
    fn test_serialize_value_response() {
        ...
        assert_eq!(decoded.status, ResponseStatus::Success as i32);
        
        match decoded.response {
            Some(ResponseType::Value(value_response)) => {
    ...
    fn test_serialize_error_response() {
        ...
        assert_eq!(decoded.status, ResponseStatus::Error as i32);
        
        match decoded.response {
            Some(ResponseType::Error(error_response)) => {
    ...
    fn test_serialize_batch_response() {
        ...
        let serialized = serializer.serialize_batch_response(responses, request_ids).unwrap();
        ...
        match decoded.response {
            Some(ResponseType::Batch(batch_response)) => {
    ...
    fn test_protocol_header_format() {
        ...
        let serialized = serializer.serialize_response(response, "ping_test".to_string()).unwrap();
