...
use super::{ToonPacket, ToonType, ToonFlags, TOON_MAGIC, TOON_VERSION};
...
impl ToonNegotiator {
    ...
    pub fn create_negotiation_request(&self) -> BytesMut {
        ...
        caps_obj.insert("max_packet_size".to_string(), 
                       ToonType::UInt32(self.capabilities.max_packet_size as u32));
        caps_obj.insert("supported_types".to_string(), 
                       ToonType::Array(self.capabilities.supported_types.iter()
                                     .map(|&t| ToonType::UInt8(t)).collect()));
    ...
    pub fn process_negotiation_request(&self, request: &[u8]) -> (ToonNegotiationResult, Option<BytesMut>) {
        if request.len() < 6 {
            return (ToonNegotiationResult::Failed("Request too short".to_string()), None);
        }
        ...
        if version != TOON_VERSION {
            return (ToonNegotiationResult::Failed(
    ...
    pub fn process_negotiation_response(&self, response: &[u8]) -> ToonNegotiationResult {
        if response.len() < 14 { // TOON + version + flags + ACCEPTED
        ...
        if version != TOON_VERSION {
            return ToonNegotiationResult::Failed(
                format!("Unsupported version in response: {}", version)
            );
        }
...
mod tests {
    ...
    fn test_negotiation_process() {
        ...
        match result {
            ToonNegotiationResult::ToonAccepted { version, flags, capabilities } => {
    ...
    fn test_protocol_detection() {
        ...
        assert_eq!(ToonProtocolDetector::detect_protocol(b"CRAB123"), "Protobuf");
    ...
    fn test_protocol_efficiency() {
        ...
        assert_eq!(ToonProtocolDetector::get_protocol_efficiency("Protobuf"), 70);
    ...
    fn test_capabilities_encoding() {
        ...
        assert_eq!(decoded.max_packet_size, negotiator.capabilities.max_packet_size);
        assert_eq!(decoded.supported_types, negotiator.capabilities.supported_types);
    }
}