    ...
    pub fn optimize_value(value: &ToonType) -> ToonType {
        match value {
            ...
            ToonType::Array(arr) => {
                ...
                let optimized: Vec<ToonType> =
                    arr.iter().map(|item| Self::optimize_value(item)).collect();
                ToonType::Array(optimized)
            }
            ToonType::Object(obj) => {
                // Optimize each value in the object
                let optimized: HashMap<String, ToonType> = obj
                    .iter()
...
impl ToonArrayBuilder {
    ...
    pub fn push_int(mut self, value: i64) -> Self {
        self.items
            .push(ToonTypeAnalyzer::optimize_value(&ToonType::Int64(value)));
        self
    }
...
impl ToonObjectBuilder {
    ...
    pub fn field<K: Into<String>>(mut self, key: K, value: ToonType) -> Self {
        self.fields
            .insert(key.into(), ToonTypeAnalyzer::optimize_value(&value));
        self
    }
    ...
    pub fn field_int<K: Into<String>>(mut self, key: K, value: i64) -> Self {
        self.fields.insert(
            key.into(),
            ToonTypeAnalyzer::optimize_value(&ToonType::Int64(value)),
        );
        self
    }
    ...
    pub fn field_string<K: Into<String>, V: Into<String>>(mut self, key: K, value: V) -> Self {
        self.fields
            .insert(key.into(), ToonType::String(value.into()));
        self
    }

    pub fn field_bytes<K: Into<String>, V: Into<Bytes>>(mut self, key: K, value: V) -> Self {
        self.fields
            .insert(key.into(), ToonType::Bytes(value.into()));
        self
    }
    ...
    pub fn from_json_value(value: &serde_json::Value) -> ToonType {
        match value {
            ...
            serde_json::Value::Array(arr) => {
                let toon_array: Vec<ToonType> =
                    arr.iter().map(|v| Self::from_json_value(v)).collect();
                ToonType::Array(toon_array)
            }
            serde_json::Value::Object(obj) => {
                let toon_object: HashMap<String, ToonType> = obj
                    .iter()
        ...
        match value {
            ...
            ToonType::Array(arr) => {
                let json_array: Vec<serde_json::Value> =
                    arr.iter().map(|v| Self::to_json_value(v)).collect();
