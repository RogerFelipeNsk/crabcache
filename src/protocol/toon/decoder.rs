...
use super::{ToonPacket, ToonType, ToonFlags, StringInterner, TOON_MAGIC, TOON_VERSION};
use bytes::Bytes;
...
impl ToonDecoder {
    ...
    fn decode_value(&mut self, bytes: &[u8], cursor: &mut usize, use_interning: bool) -> Result<ToonType, String> {
        if *cursor >= bytes.len() {
...
impl ToonDecoder {
    ...
    pub fn decode_to_command(&mut self, bytes: &[u8]) -> Result<crate::protocol::commands::Command, String> {
    ...
    fn extract_bytes(&mut self, obj: &HashMap<String, ToonType>, key: &str) -> Result<Bytes, String> {
        match obj.get(key) {
