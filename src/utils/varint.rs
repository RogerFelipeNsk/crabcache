...
pub fn decode_varint(bytes: &[u8]) -> crate::Result<(u64, usize)> {
    ...
    for (i, &byte) in bytes.iter().enumerate().take(10) { // Max 10 bytes for u64
        if shift >= 64 {
...
pub fn decode_varint_bytes(buf: &mut Bytes) -> Option<u64> {
    ...
    for i in 0..10 { // Max 10 bytes for u64
        if buf.is_empty() {
            return None;
        }
...
mod tests {
    ...
    fn test_varint_roundtrip() {
        ...
        for value in test_values {
            ...
            assert_eq!(consumed, bytes.len(), "Consumed bytes mismatch for value {}", value);
        }
    }
    ...
    fn test_backward_compatibility() {
        ...
        for value in test_values {
            ...
            assert_eq!(old_result, Some(new_result), "Decode mismatch for {}", value);
        }
    }
}