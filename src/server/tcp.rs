...
use crate::Config;
use crate::protocol::{ProtocolParser, ProtocolSerializer, BinaryProtocol, PipelineProcessor, PipelineResponseBatch};
...
use crate::security::{SecurityManager, SecurityContext, SecurityCheckResult, AuthManager, RateLimiter, IpFilter};
...
use connection_metrics::{ConnectionMetrics, ConnectionGuard, CommandTimer};
...
impl ShardManagerType {
    ...
    pub async fn process_batch(&self, commands: Vec<crate::protocol::commands::Command>) -> Vec<Response> {
...
impl TcpServer {
    ...
    pub async fn new(config: Config) -> crate::Result<Self> {
            ...
            match WALShardManager::new_with_recovery(
                ...
                Ok((wal_manager, recovery_stats)) => {
                    if let Some(stats) = recovery_stats {
                        info!("WAL recovery completed: {} entries recovered in {}ms", 
        ...
        info!("  - {} shards, {}B per shard", num_shards, max_memory_per_shard);
        match &shard_manager {
    ...
    fn create_security_manager(config: &Config) -> Result<SecurityManager, Box<dyn std::error::Error + Send + Sync>> {
        // Create authentication manager
        let auth_manager = if config.security.enable_auth {
            if let Some(ref token) = config.security.auth_token {
                Some(AuthManager::with_token(token.clone(), "default_user".to_string()))
            } else {
    ...
    pub async fn start_with_metrics(&self, metrics_port: u16) -> crate::Result<()> {
        ...
        info!("  - http://localhost:{}/health (Health check)", metrics_port);
                ...
                Ok((stream, addr)) => {
                    ...
                    tokio::spawn(async move {
                        if let Err(e) = Self::handle_connection_extreme_optimized(
                            stream, config, router, shard_manager, buffer_pool, metrics, shared_metrics, security_manager, addr, pipeline_enabled, max_pipeline_batch_size
                        ).await {
    ...
    async fn handle_connection_extreme_optimized(
        ...
        match security_manager.check_connection(&security_context).await {
            ...
            result => {
                warn!("Connection from {} blocked: {}", client_addr, result.error_message());
        ...
        debug!("Connection established with pipelining: enabled={}", pipeline_enabled);
        ...
        loop {
            ...
            if pipeline_enabled && pipeline_processor.is_some() {
                ...
                match processor.parse_batch(&buffer[..n]) {
                    Ok(batch) => {
                        debug!("Processing pipeline batch with {} commands", batch.commands.len());
                        
                        // Security check for each command in batch
                        let mut allowed_commands = Vec::new();
                        for command in batch.commands {
                            let auth_result = security_manager.authenticate_command(&mut security_context, None);
                        ...
                        if !allowed_commands.is_empty() {
                            ...
                            match processor.serialize_response_batch(&response_batch) {
                                Ok(response_bytes) => {
                                    ...
                                    debug!("Sent pipeline response: {} commands processed", response_batch.responses.len());
                                }
                                Err(e) => {
                    ...
                    Err(_) => {
                        ...
                        if let Err(e) = Self::process_single_command(
                            &buffer[..n], &shard_manager, &mut security_context, &security_manager,
                            &mut stream, &mut response_buffer, &metrics, client_addr
                        ).await {
    ...
    async fn process_single_command(
        ...
        if !auth_result.is_allowed() {
            warn!("Command from {} blocked: {}", client_addr, auth_result.error_message());
        ...
        stream.write_all(&response_bytes).await?;
        debug!("Processed single command, sent {} bytes", response_bytes.len());
