...
use super::{Shard};
use crate::eviction::{TinyLFU, EvictionConfig, MemoryPressureMonitor, MemoryPressureCoordinator, EvictionPolicy};
...
impl EvictionShardManager {
    ...
    pub async fn process_command(&self, command: Command) -> Response {
        ...
        let response = match &command {
            Command::Put { key, value, ttl } => {
                ...
                let response = shard.put_with_eviction(
            ...
            Command::Del { key } => {
                let shard = self.get_shard_for_key(key);
                let response = shard.delete_with_eviction(&String::from_utf8_lossy(key)).await;
    ...
    async fn get_eviction_stats(&self) -> Response {
        ...
        stats.insert("total_memory_usage".to_string(), 
                    serde_json::Value::Number(coordinator.total_usage().into()));
        stats.insert("total_memory_limit".to_string(), 
                    serde_json::Value::Number(coordinator.total_limit().into()));
        stats.insert("overall_usage_ratio".to_string(), 
                    serde_json::Value::Number(serde_json::Number::from_f64(coordinator.overall_usage_ratio()).unwrap()));
        ...
        for (i, shard) in self.shards.iter().enumerate() {
            ...
            shard_stat.insert("hit_ratio".to_string(), 
                            serde_json::Number::from_f64(eviction_metrics.hit_ratio).unwrap().into());
            shard_stat.insert("evictions".to_string(), eviction_metrics.evictions.into());
            shard_stat.insert("admissions_accepted".to_string(), eviction_metrics.admissions_accepted.into());
            shard_stat.insert("admissions_rejected".to_string(), eviction_metrics.admissions_rejected.into());
            shard_stat.insert("admission_ratio".to_string(), 
                            serde_json::Number::from_f64(eviction_metrics.admission_ratio).unwrap().into());
            ...
            if !shards_needing_eviction.is_empty() {
                ...
                for shard_id in shards_needing_eviction {
                    if let Some(shard) = shards.get(shard_id) {
                        ...
                        // Perform eviction
                        let evicted: Vec<(String, Vec<u8>)> = shard.force_eviction(items_to_evict).await;
                        
                        if !evicted.is_empty() {
                            info!("Evicted {} items from shard {}", evicted.len(), shard_id);
                            
                            // Update memory usage
                            let freed_bytes: usize = evicted.iter()
    ...
    pub async fn get_metrics(&self) -> serde_json::Value {
        ...
        for (i, shard) in self.shards.iter().enumerate() {
            ...
            shard_stat.insert("admission_ratio".to_string(), 
                            serde_json::Number::from_f64(eviction_metrics.admission_ratio).unwrap().into());
            shard_stat.insert("memory_usage".to_string(), shard.memory_monitor.current_usage().into());
            shard_stat.insert("memory_limit".to_string(), shard.memory_monitor.memory_limit().into());
            shard_stat.insert("memory_pressure".to_string(), 
                            serde_json::Number::from_f64(shard.memory_monitor.pressure_level()).unwrap().into());
            
            shard_stats.push(serde_json::Value::Object(shard_stat));
        }
        
        stats.insert("eviction".to_string(), serde_json::json!({
...
impl EvictionShard {
    ...
    pub async fn put_with_eviction(
        ...
        match self.try_tinylfu_put(&key, &value) {
            ...
            Err(e) => {
                warn!("TinyLFU PUT failed for key '{}': {}, falling back to regular shard", key, e);
    ...
    pub async fn get_with_eviction(&self, key: &str) -> Response {
        ...
        match self.try_tinylfu_get(key) {
            ...
            Ok(None) => {
                // Try fallback shard
                let cmd = Command::Get { key: Bytes::from(key.to_string()) };
            ...
            Err(e) => {
                warn!("TinyLFU GET failed for key '{}': {}, falling back to regular shard", key, e);
    ...
    pub async fn delete_with_eviction(&self, key: &str) -> Response {
        ...
        match self.try_tinylfu_remove(key) {
            ...
            Err(e) => {
                warn!("TinyLFU DELETE failed for key '{}': {}, continuing with fallback", key, e);
            }
        }
        
        // Also try fallback shard
        let cmd = Command::Del { key: Bytes::from(key.to_string()) };
    ...
    fn try_tinylfu_put(&self, key: &str, value: &[u8]) -> Result<Option<(String, Vec<u8>)>, String> {
        match self.eviction_cache.try_write() {
            Ok(mut cache) => {
                Ok(cache.put(key.to_string(), value.to_vec()))
            }
    ...
    pub async fn force_eviction(&self, count: usize) -> Vec<(String, Vec<u8>)> {
        match self.eviction_cache.try_write() {
            Ok(mut cache) => {
                cache.evict_items(count)
            }
            Err(e) => {
                error!("Failed to acquire lock for force eviction on shard {}: {}", self.id, e);
...
mod tests {
    ...
    async fn test_put_get_with_eviction() {
        ...
        // Put some items
        let response = shard.put_with_eviction("key1".to_string(), b"value1".to_vec(), None).await;
        assert!(matches!(response, Response::Ok));
        
        let response = shard.put_with_eviction("key2".to_string(), b"value2".to_vec(), None).await;
        assert!(matches!(response, Response::Ok));
        ...
        let response = shard.put_with_eviction("key3".to_string(), b"value3".to_vec(), None).await;
        assert!(matches!(response, Response::Ok));
    }
    ...
    async fn test_delete_with_eviction() {
        ...
        // Put and delete
        shard.put_with_eviction("key1".to_string(), b"value1".to_vec(), None).await;
    ...
    async fn test_force_eviction() {
        ...
        // Fill cache
        for i in 0..5 {
            shard.put_with_eviction(format!("key{}", i), format!("value{}", i).into_bytes(), None).await;
        }
