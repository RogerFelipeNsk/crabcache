...
use crate::store::{ZeroCopyStore, CrabCacheLockFreeMap};
...
impl OptimizedShardManager {
    ...
    pub fn new(num_shards: usize, max_memory_per_shard: usize) -> Self {
        info!("Creating OptimizedShardManager with {} shards, {}B per shard", num_shards, max_memory_per_shard);
        
        let metrics = crate::metrics::create_shared_metrics(num_shards);
        
        let shards: Vec<Arc<OptimizedShard>> = (0..num_shards)
            .map(|id| Arc::new(OptimizedShard::new(id, max_memory_per_shard, Arc::clone(&metrics))))
            .collect();
    ...
    pub fn get_shard_for_key_simd(&self, key: &[u8]) -> &Arc<OptimizedShard> {
        ...
        debug!("SIMD Key {} -> shard {}", String::from_utf8_lossy(key), shard_index);
    ...
    async fn get_detailed_stats(&self) -> Response {
        if let Ok(metrics) = self.metrics.try_read() {
            ...
            let json = serde_json::to_string_pretty(&stats).unwrap_or_else(|_| "Error serializing stats".to_string());
            Response::Stats(json)
        } else {
    ...
    pub async fn bulk_put(&self, entries: Vec<(Bytes, Bytes)>) -> usize {
        ...
        let mut shard_groups: std::collections::HashMap<usize, Vec<(Bytes, Bytes)>> = std::collections::HashMap::new();
        ...
        for (key, value) in entries {
            ...
            shard_groups.entry(shard_idx).or_insert_with(Vec::new).push((key, value));
        }
    ...
    pub fn set_zero_copy_enabled(&mut self, enabled: bool) {
        ...
        info!("Zero-copy operations: {}", if enabled { "enabled" } else { "disabled" });
    }
    
    pub fn set_simd_enabled(&mut self, enabled: bool) {
        self.simd_enabled = enabled;
        info!("SIMD operations: {}", if enabled { "enabled" } else { "disabled" });
    }
    
    pub fn set_lockfree_enabled(&mut self, enabled: bool) {
        self.lockfree_enabled = enabled;
        info!("Lock-free operations: {}", if enabled { "enabled" } else { "disabled" });
    }
}
...
impl OptimizedShard {
    ...
    pub async fn put_zero_copy(&self, key: Bytes, value: Bytes, ttl: Option<u64>) -> Response {
        ...
        match self.zero_copy_store.put_zero_copy(key.clone(), value.clone()) {
            Ok(()) => {
                ...
                if let Some(ttl_seconds) = ttl {
                    ...
                    let cmd = Command::Expire { key: key.clone(), ttl: ttl_seconds };
    ...
    pub async fn expire_optimized(&self, key: &Bytes, ttl: u64) -> Response {
        ...
        let cmd = Command::Expire { key: key.clone(), ttl };
        self.regular_shard.process_command(cmd).await
    }
