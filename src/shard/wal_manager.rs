...
use crate::wal::{WALWriter, WALReader, WALConfig, Operation, WALReplayTarget, RecoveryStats};
...
impl WALShardManager {
    ...
    async fn recover_from_wal(&self) -> Result<RecoveryStats, Box<dyn std::error::Error + Send + Sync>> {
    ...
    /// Log command to WAL
    async fn log_command_to_wal(&self, command: &Command) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    fn get_shard_id(&self, key: &Bytes) -> usize {
        self.eviction_manager.get_shard_id(&String::from_utf8_lossy(key))
    }
...
impl WALReplayTarget for WALShardManager {
    ...
    async fn replay_operation(&self, shard_id: usize, operation: &Operation) -> Result<(), Self::Error> {
        debug!("Replaying WAL operation: shard={}, op={:?}", shard_id, operation);
...
mod tests {
    ...
    async fn test_wal_recovery() {
        ...
        let (manager, stats) = WALShardManager::new_with_recovery(
            2,
            1024 * 1024,
            eviction_config,
            Some(wal_config),
