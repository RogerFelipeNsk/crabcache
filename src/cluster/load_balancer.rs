...
impl LoadBalancer {
    ...
    pub async fn select_node(&self, available_nodes: &[ClusterNode]) -> ClusterResult<NodeId> {
        ...
        let selected_node = match &self.strategy {
            ...
            LoadBalancingStrategy::Adaptive { primary, fallback, switch_threshold } => {
                self.select_adaptive(&healthy_nodes, primary, fallback, *switch_threshold).await
            }
        }?;
    ...
    async fn select_least_connections(&self, nodes: &[ClusterNode]) -> ClusterResult<NodeId> {
        ...
        let mut best_node = nodes[0].id;
        let mut best_connections = metrics.get(&best_node)
            .map(|m| m.active_connections)
            .unwrap_or(0);
        
        for node in &nodes[1..] {
            let connections = metrics.get(&node.id)
                .map(|m| m.active_connections)
                .unwrap_or(0);
    ...
    async fn select_least_response_time(&self, nodes: &[ClusterNode]) -> ClusterResult<NodeId> {
        ...
        let mut best_node = nodes[0].id;
        let mut best_response_time = metrics.get(&best_node)
            .map(|m| m.avg_response_time)
            .unwrap_or(0.0);
        
        for node in &nodes[1..] {
            let response_time = metrics.get(&node.id)
                .map(|m| m.avg_response_time)
                .unwrap_or(0.0);
    ...
    async fn select_resource_based(&self, nodes: &[ClusterNode]) -> ClusterResult<NodeId> {
        ...
        let mut best_node = nodes[0].id;
        let mut best_score = metrics.get(&best_node)
            .map(|m| m.load_score())
            .unwrap_or(0.0);
        
        for node in &nodes[1..] {
            let score = metrics.get(&node.id)
                .map(|m| m.load_score())
                .unwrap_or(0.0);
    ...
    async fn select_consistent_hash_with_load(&self, nodes: &[ClusterNode]) -> ClusterResult<NodeId> {
    ...
    async fn select_adaptive(
        ...
        let avg_load = if metrics.is_empty() { 0.0 } else { total_load / metrics.len() as f64 };
        ...
        match strategy {
            ...
            LoadBalancingStrategy::WeightedRoundRobin => self.select_weighted_round_robin(nodes).await,
            LoadBalancingStrategy::LeastConnections => self.select_least_connections(nodes).await,
            LoadBalancingStrategy::LeastResponseTime => self.select_least_response_time(nodes).await,
            LoadBalancingStrategy::ResourceBased => self.select_resource_based(nodes).await,
            LoadBalancingStrategy::ConsistentHashWithLoad => self.select_consistent_hash_with_load(nodes).await,
    ...
    async fn update_metrics(&self, selected_node: NodeId, decision_time: Duration) {
        ...
        *metrics.requests_per_node.entry(selected_node).or_insert(0) += 1;
        ...
        metrics.avg_decision_time = (metrics.avg_decision_time * (metrics.balancing_decisions - 1) as f64 + 
                                   decision_time_ms) / metrics.balancing_decisions as f64;
        ...
        if !metrics.requests_per_node.is_empty() {
            ...
            let variance: f64 = metrics.requests_per_node.values()
                .map(|&count| {
