...
impl ConsistentHashRing {
    ...
    pub fn add_node(&mut self, node: ClusterNode) -> Vec<ShardMigration> {
        ...
        info!("Added node {} with {} virtual nodes, {} migrations needed", 
              node_id, self.virtual_nodes, migrations.len());
    ...
    pub fn remove_node(&mut self, node_id: NodeId) -> Vec<ShardMigration> {
        ...
        info!("Removed node {}, {} migrations needed", node_id, migrations.len());
    ...
    fn calculate_migrations_for_new_node(&self, new_node_id: NodeId) -> Vec<ShardMigration> {
        ...
        for i in 0..self.virtual_nodes {
            ...
            let next_node = self.ring.range((hash + 1)..)
                .next()
            ...
            if let Some(from_node) = next_node {
                if from_node != new_node_id {
                    ...
                    let migration = ShardMigration {
                        migration_id: format!("migration_{}_{}_to_{}", 
    ...
    fn calculate_migrations_for_removed_node(&self, removed_node_id: NodeId) -> Vec<ShardMigration> {
        let mut migrations = Vec::new();
        
        // Find all virtual nodes that belonged to the removed node
        let removed_hashes: Vec<u64> = self.ring.iter()
        ...
        for hash in removed_hashes {
            ...
            let next_node = self.ring.range((hash + 1)..)
                ...
                .or_else(|| self.ring.iter().find(|(_, &node_id)| node_id != removed_node_id))
                .map(|(_, &node_id)| node_id);
...
impl AutoShardingManager {
    ...
    pub fn update_migration_progress(&mut self, migration_id: &str, progress: f64) -> ClusterResult<()> {
        if let Some(migration) = self.active_migrations.get_mut(migration_id) {
            migration.progress = progress.clamp(0.0, 1.0);
            debug!("Migration {} progress: {:.1}%", migration_id, progress * 100.0);
            Ok(())
        } else {
            Err(ClusterError::MigrationFailed { 
                migration_id: migration_id.to_string() 
...
mod tests {
    ...
    fn test_key_distribution() {
        ...
        for count in distribution.values() {
            assert!(*count > avg * 70 / 100, "Count {} too low, avg {}", count, avg);
            assert!(*count < avg * 130 / 100, "Count {} too high, avg {}", count, avg);
        }
    }
    ...
    fn test_load_distribution() {
        ...
        assert!((total_load - 1.0).abs() < 0.001, "Total load {} != 1.0", total_load);
    }
    ...
    fn test_auto_sharding_manager() {
        ...
        if let Some(migration) = migration {
            ...
            // Update progress
            manager.update_migration_progress(&migration_id, 0.5).unwrap();
