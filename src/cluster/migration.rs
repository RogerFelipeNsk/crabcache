...
use crate::cluster::hash_ring::{MigrationStatus, ShardMigration};
use crate::cluster::{ClusterError, ClusterResult, NodeId};
...
impl MigrationExecutor {
    ...
    pub fn new(config: MigrationConfig) -> Self {
        ...
        let concurrency_limiter =
            Arc::new(Semaphore::new(config.max_concurrent_migrations as usize));
    ...
    async fn execute_migration_task(
        ...
        while attempts < config.max_retries {
            ...
            match Self::perform_migration(&migration, &mut progress, &progress_sender, &config)
                .await
            {
                Ok(_) => {
                ...
                Err(e) => {
                    ...
                    warn!(
                        ...
                        migration_id, attempts, last_error
                    );
        ...
        error!(
            ...
            migration_id, config.max_retries
        );
    ...
    async fn perform_migration(
        ...
        info!(
            "Phase 1: Scanning keys for migration {}",
        ...
        info!(
            "Phase 2: Transferring {} keys for migration {}",
            total_keys, migration.migration_id
        );
        ...
        while transferred_keys < total_keys {
            let batch_size = config
                .batch_size
                .min((total_keys - transferred_keys) as u32) as usize;
        ...
        info!(
            "Phase 4: Cleaning up source data for migration {}",
            migration.migration_id
        );
    ...
    async fn scan_keys_for_migration(migration: &ShardMigration) -> ClusterResult<u64> {
        ...
        debug!(
            "Scanning keys for migration {} in range {:?}",
    ...
    async fn transfer_batch(
        ...
        debug!(
            "Transferring batch {} ({} keys) from {} to {}",
            batch.batch_id,
            batch.keys.len(),
            migration.from_node,
            migration.to_node
    ...
    async fn cleanup_source_data(migration: &ShardMigration) -> ClusterResult<()> {
        ...
        debug!(
            "Cleaning up source data for migration {}",
            migration.migration_id
        );
    ...
    async fn start_progress_monitor(&self) {
        let mut progress_receiver = self
            .progress_receiver
            .write()
            .await
            .take()
