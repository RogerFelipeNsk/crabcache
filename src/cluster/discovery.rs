...
use crate::cluster::{
    ClusterConfig, ClusterError, ClusterMetrics, ClusterNode, ClusterResult, NodeId, NodeStatus,
};
use dashmap::DashMap;
...
impl ServiceDiscovery {
        ...
        match message {
            DiscoveryMessage::NodeAnnouncement { node, cluster_name } => {
                if cluster_name == &config.cluster_name {
                    ...
                    warn!(
                        "Ignoring announcement from different cluster: {}",
                        cluster_name
                    );
                }
            }
            ...
            DiscoveryMessage::JoinRequest { node, cluster_name } => {
                if cluster_name == &config.cluster_name {
                    ...
                    warn!(
                        "Rejecting join request from different cluster: {}",
                        cluster_name
                    );
                }
            }
    ...
    async fn handle_connection(
        ...
        match &message {
            DiscoveryMessage::StatusRequest => {
                ...
                let serialized =
                    bincode::serialize(&response).map_err(|e| ClusterError::NetworkError {
    ...
    async fn send_message_to_stream(
        ...
        let serialized = bincode::serialize(message).map_err(|e| ClusterError::NetworkError {
            source: std::io::Error::new(std::io::ErrorKind::InvalidData, e.to_string()),
        })?;
    ...
    async fn receive_message_from_stream(
        &self,
        stream: &mut TcpStream,
    ) -> ClusterResult<DiscoveryMessage> {
        ...
        // Deserialize message
        let message = bincode::deserialize(&buffer).map_err(|e| ClusterError::NetworkError {
            source: std::io::Error::new(std::io::ErrorKind::InvalidData, e.to_string()),
        })?;
    ...
    async fn receive_message_from_stream_static(
        stream: &mut TcpStream,
    ) -> ClusterResult<DiscoveryMessage> {
        // Read message length first (4 bytes)
        let mut len_bytes = [0u8; 4];
            ...
            loop {
                ...
                if !nodes.is_empty() {
                    let total_throughput: f64 =
                        nodes.iter().map(|n| n.stats.current_ops_per_sec).sum();
                    metrics.cluster_throughput = total_throughput;

                    let loads: Vec<f64> = nodes.iter().map(|n| n.load_factor()).collect();
