...
impl RateLimiter {
    ...
    async fn cleanup_old_buckets(&self) {
        ...
        buckets.retain(|_key, bucket| now.duration_since(bucket.last_refill) < cleanup_threshold);
...
mod tests {
    ...
    async fn test_rate_limiter_different_keys() {
        ...
        assert_eq!(
            rate_limiter.check_rate("client1").await,
            RateLimitResult::Allowed
        );
        assert_eq!(
            rate_limiter.check_rate("client2").await,
            RateLimitResult::Allowed
        );
        assert_eq!(
            rate_limiter.check_rate("client1").await,
