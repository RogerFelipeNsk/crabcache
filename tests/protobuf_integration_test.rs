...
async fn test_protobuf_serialization_roundtrip() {
    ...
    for (name, response) in test_cases {
        ...
        let serialized = serializer.serialize_response(response, request_id.clone()).unwrap();
        ...
        let decoded = crabcache::protocol::protobuf::generated::CrabCacheResponse::decode(message_data);
...
async fn test_batch_serialization() {
    ...
    let message_data = &serialized[9..];
    let decoded = crabcache::protocol::protobuf::generated::CrabCacheResponse::decode(message_data).unwrap();
    
    match decoded.response {
        Some(crabcache::protocol::protobuf::generated::crab_cache_response::Response::Batch(batch)) => {
...
async fn test_message_size_limits() {
    ...
    match result.unwrap_err() {
        crabcache::protocol::protobuf::ProtobufError::MessageTooLarge { size: 200, max: 100 } => {},
...
async fn test_schema_registry_basic() {
    ...
    registry.register_schema("test_schema".to_string(), schema_data.clone()).unwrap();
