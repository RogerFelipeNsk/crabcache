...
async fn test_multiple_keys_different_ttls() {
    ...
    for (key, _) in &keys_ttls {
        ...
        let response = router.process_command(get_cmd).await;
        assert!(matches!(response, Response::Value(_)), "Key {} should be available initially", key);
    }
    ...
    let response = router.process_command(get_cmd).await;
    assert_eq!(response, Response::Null, "key1 should be expired after 4 seconds");
    ...
    for (key, _) in &keys_ttls {
        ...
        let response = router.process_command(get_cmd).await;
        assert_eq!(response, Response::Null, "Key {} should be expired after 6 seconds", key);
    }
}
