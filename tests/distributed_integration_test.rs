...
use crabcache::cluster::{
    ClusterNode, NodeCapabilities, NodeId, ConsistentHashRing,
    LoadBalancer, LoadBalancingStrategy, DistributedPipelineManager,
};
...
async fn test_consistent_hash_ring_distribution() {
    ...
    for count in distribution.values() {
        assert!(*count > avg * 60 / 100, "Distribution too uneven: {}", count);
        assert!(*count < avg * 140 / 100, "Distribution too uneven: {}", count);
    }
...
async fn test_distributed_pipeline_processing() {
    ...
    // Test commands
    let commands = vec![
        PipelineCommand::Set { key: "test_key_1".to_string(), value: "value1".to_string() },
        PipelineCommand::Set { key: "test_key_2".to_string(), value: "value2".to_string() },
        PipelineCommand::Get { key: "test_key_1".to_string() },
        PipelineCommand::Get { key: "test_key_2".to_string() },
        PipelineCommand::Ping,
        PipelineCommand::Info,
    ];
    
    // Process commands
    let result = pipeline_manager.process_distributed_batch(commands.clone()).await;
    
    assert!(result.is_ok(), "Distributed processing should succeed");
    
    let response_batch = result.unwrap();
    assert_eq!(response_batch.responses.len(), commands.len(), 
...
async fn test_hash_ring_node_addition_removal() {
    ...
    assert_eq!(distribution_2_nodes.len(), 2, "Should distribute across 2 nodes");
    ...
    assert!(!migrations.is_empty(), "Should have migrations when adding node");
    ...
    assert_eq!(distribution_3_nodes.len(), 3, "Should distribute across 3 nodes");
    
    // Remove a node
    let removal_migrations = hash_ring.remove_node(node2.id);
    assert!(!removal_migrations.is_empty(), "Should have migrations when removing node");
    ...
    assert_eq!(distribution_after_removal.len(), 2, "Should distribute across 2 nodes after removal");
}
...
async fn test_routing_strategies() {
    ...
    let strategies = vec![
        ...
        RoutingStrategy::Hybrid { hash_weight: 0.7, load_weight: 0.3 },
    ...
    for strategy in strategies {
        ...
        let commands = vec![
            PipelineCommand::Get { key: "test_key".to_string() },
            PipelineCommand::Set { key: "another_key".to_string(), value: "value".to_string() },
        ];
...
async fn test_fault_tolerance_simulation() {
    ...
    let commands = vec![
        PipelineCommand::Set { key: "fault_test".to_string(), value: "data".to_string() },
        PipelineCommand::Get { key: "fault_test".to_string() },
    ];
    ...
    assert!(result_after_failure.is_ok(), "Should handle node failure gracefully");
...
async fn test_cluster_metrics_collection() {
    ...
    assert!((total_load - 1.0).abs() < 0.1, "Total load should be close to 1.0");
}